...
class CROPDataset(BaseDataSet):
	"""
	2020 CCF BCDI dataset
	https://aistudio.baidu.com/datasetdetail/55865/0
	"""

	def __init__(self, **kwargs):
		self.num_classes = 2
		self.palette = palette.get_voc_palette(self.num_classes)
		super(CROPDataset, self).__init__(**kwargs)
...
class CROP(BaseDataLoader):
	def __init__(self, data_dir, batch_size, split, crop_size=None, base_size=None, scale=True, num_workers=1,
		...
		if split in ["train", "trainval", "val", "test"]:
			self.dataset = CROPDataset(**kwargs)
		else:
			raise ValueError(f"Invalid split name {split}")
		super(CROP, self).__init__(self.dataset, batch_size, shuffle, num_workers, val_split)
