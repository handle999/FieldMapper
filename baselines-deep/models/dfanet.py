...
class DFANet(nn.Module):
	def __init__(self, num_classes=2, channel_cfg=None, decoder_channel=64, freeze_bn=False, **_):
		super(DFANet, self).__init__()
		if channel_cfg is None:
			channel_cfg = [[8, 48, 96], [240, 144, 288], [240, 144, 288]]
...
if __name__ == "__main__":
	device = "cuda:0"
	model = DFANet(num_classes=2).to(device)
	total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
	print(f"Total trainable parameters: {total_params:,}")  # 2,177,424
