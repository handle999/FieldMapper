...
class SEANet(nn.Module):
    def __init__(self, num_classes=2, add_output=True, bilinear=False, num_filters=32, is_deconv=False,
        ...
        # Define downsampling layers with 1x1 convolution (adjusted for num_classes)
        self.conv1_1_down = nn.Conv2d(64, num_classes, 1, padding=0)
        self.conv1_2_down = nn.Conv2d(64, num_classes, 1, padding=0)
        self.conv2_1_down = nn.Conv2d(128, num_classes, 1, padding=0)
        self.conv2_2_down = nn.Conv2d(128, num_classes, 1, padding=0)
        self.conv3_1_down = nn.Conv2d(256, num_classes, 1, padding=0)
        self.conv3_2_down = nn.Conv2d(256, num_classes, 1, padding=0)
        self.conv3_3_down = nn.Conv2d(256, num_classes, 1, padding=0)
        self.conv4_1_down = nn.Conv2d(512, num_classes, 1, padding=0)
        self.conv4_2_down = nn.Conv2d(512, num_classes, 1, padding=0)
        self.conv4_3_down = nn.Conv2d(512, num_classes, 1, padding=0)
        self.conv5_1_down = nn.Conv2d(512, num_classes, 1, padding=0)
        self.conv5_2_down = nn.Conv2d(512, num_classes, 1, padding=0)
        self.conv5_3_down = nn.Conv2d(512, num_classes, 1, padding=0)

        # Define score layers for each downsampled output (adjusted for num_classes)
        self.score_dsn1 = nn.Conv2d(num_classes, num_classes, 1)
        self.score_dsn2 = nn.Conv2d(num_classes, num_classes, 1)
        self.score_dsn3 = nn.Conv2d(num_classes, num_classes, 1)
        self.score_dsn4 = nn.Conv2d(num_classes, num_classes, 1)
        self.score_dsn5 = nn.Conv2d(num_classes, num_classes, 1)

        # Final score layer (adjusted for num_classes)
        self.score_final = nn.Conv2d(5 * num_classes, num_classes, 1)

        # Define upsampling weights
        self.weight_deconv2 = make_bilinear_weights(4, num_classes).cuda()
        self.weight_deconv3 = make_bilinear_weights(8, num_classes).cuda()
        self.weight_deconv4 = make_bilinear_weights(16, num_classes).cuda()
        self.weight_deconv5 = make_bilinear_weights(16, num_classes).cuda()

        self.add_output = add_output
        if add_output:
            self.conv_final1 = nn.Conv2d(64, num_classes, 1)
            self.conv_final2 = nn.Conv2d(64, num_classes, 1)

    def forward(self, x):
        img_H, img_W = x.shape[2], x.shape[3]

        # VGG-like convolution layers
        ...
        # Compute scores for each downsampled output
        ...
        # Crop outputs to original image size
        ...
        # Fuse the outputs and apply final score layer
...
if __name__ == "__main__":
    device = "cuda:0"
    model = SEANet(num_classes=2).to(device)
    total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"Total trainable parameters: {total_params:,}")  # 32,211,730
