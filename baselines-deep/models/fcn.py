...
class FCN8(BaseModel):
    def __init__(self, num_classes, pretrained=True, freeze_bn=False, freeze_backbone=None, **_):
...
if __name__ == "__main__":
    device = "cuda:0"
    model = FCN8(num_classes=2).to(device)
    total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"Total trainable parameters: {total_params:,}")  # 134,270,278

    test_input = torch.randn(4, 3, 256, 256).cuda()  # 假设输入是4张 256x256 的RGB图像
    output = model(test_input)
    print(output.shape)  # 输出形状应该是 (4, 1, 256, 256)
    # print(model)
